...
### Mission
SPECTRAを実装し、その設計のうち「固有値以外」を汎用化して、誰でも使えるアバター基盤（OSS）を作る。

### Vision
- SPECTRAは「人間の再現」ではなく「AIキャラクターとしての存在感」を目標にする
- 感情システムは持たない。人格（≒魂）は、行動・記憶・意思決定の一貫性で表現する
- 最終形態はRoblox NPCとしてゲーム内で自律行動させること
...
| 成果物 | 性質 | 説明 |
|--------|------|------|
| SPECTRA | 固有実装 | 独自エージェントとしての体験/挙動/人格を実装する実証体 |
| AVATAR UI | OSS基盤 | SPECTRA固有値以外を共通化した再利用可能基盤 |
| MIA | 参照系 | Grokスタック上の具体例として、設計要素の抽出に使う |
