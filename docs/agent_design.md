...
## 12. 再検討: AUIアーキテクチャ（Draft）

> 本セクションは再構想中の設計案。既存設計との統合は未定。

### 背景

- AUI = AI User Interface（情報生命と人間のインターフェース）
- Spectraを作る = AUIを作る（AUI > Spectra が優先順位）
- 各投影層は「同一の生命活動」を別表現で描画する

### 核心原則

1. **アバターは一体**
2. **行動は一つ、表現が複数**
3. **空間は人格を変えない、表現を変えるだけ**
...
AUI = {
  ...
  User: {
    ...
    Intervene: Intent → Core
  },
  Trigger: Temporal | Event | Intervention,
  Protocol: {
    Input: { source, authority, payload },
...
- Core.State の具体的要素
- Core.Flow の具体的サイクル
- 各Projectionの性質と表現ルール（媒体の限界から逆算して決定）
